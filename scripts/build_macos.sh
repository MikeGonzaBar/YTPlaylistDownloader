...
uv run python -m PyInstaller \
  ...
  --windowed \
  --clean \
  --icon "macos/PlaylistFolderDownloader/Sources/PlaylistFolderDownloader/Resources/AppIcon.icns" \
  --add-data "src/playlist_folder_downloader/i18n:playlist_folder_downloader/i18n" \
  --add-data "src/playlist_folder_downloader/assets:playlist_folder_downloader/assets" \
  --add-data "README.md:." \
  --add-data "LICENSE:." \
